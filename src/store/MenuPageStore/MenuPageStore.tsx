...
export default class MenuPageStore implements IMenuPageStore, ILocalStore {
  ...
  async fetchMealData(): Promise<void> {
    ...
    try {
      const result = await axios({
        method: 'get',
        url: `https://api.spoonacular.com/recipes/complexSearch?addRecipeNutrition=true&number=5&apiKey=64762942fad54dbea83fb7b0803d1c37`,
      })
      runInAction(() => {
  ...
  async fetchOffsetData(): Promise<void> {
    ...
    try {
      const result = await axios({
        method: 'get',
        url: `https://api.spoonacular.com/recipes/complexSearch?addRecipeNutrition=true&number=${this._number}&offset=${this._offset}&apiKey=64762942fad54dbea83fb7b0803d1c37`,
      })
      runInAction(() => {
  ...
  async fetchRecipeByCategory(category: string): Promise<void> {
    ...
    try {
      const result = await axios({
        method: 'get',
        url: `https://api.spoonacular.com/recipes/complexSearch?type=${category}&number=${this._number}&offset=${this._offset}&addRecipeNutrition=true&apiKey=64762942fad54dbea83fb7b0803d1c37`,
      })
      runInAction(() => {
        newData = result.data.results.map((raw: any) => ({
          id: raw.id,
  ...
  async fetchRecipeByName(name: string, scrolling: boolean): Promise<void> {
    ...
    try {
      const result = await axios({
        method: 'get',
        url: `https://api.spoonacular.com/recipes/complexSearch?query=${name}&number=${this._number}&offset=${this._offset}&addRecipeNutrition=true&apiKey=64762942fad54dbea83fb7b0803d1c37`,
      })
      runInAction(() => {
        newData = result.data.results.map((raw: any) => ({
          id: raw.id,
